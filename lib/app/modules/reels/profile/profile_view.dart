...
class _ProfileViewState extends State<ProfileView> {
        ...
        builder: (controller) {
          ...
          return ListView(
            children: [
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        _statItem(
                            widget.user?.totalViews?.toString() ?? '0',
                            'Views'),
                        _buildVerticalDivider(),
              ...
              if (_selectedTab == 0) _buildPostsTab(controller),
              if (_selectedTab == 1) _buildReactionsTab(controller),
            ],
          );
        },
      ),
  ...
  Widget _buildPostsTab(ReelsController controller) {
    final String userName = widget.user?.userName ?? '';
    final userReels = controller.reelsList
        .where((r) => r.userName == userName)
        .toList();

    // userReels empty হলে static userVideos fallback
    final bool hasReels = userReels.isNotEmpty;
    final bool hasVideos = widget.user?.userVideos != null &&
        widget.user!.userVideos.isNotEmpty;
  ...
  Widget _buildReactionsTab(ReelsController controller) {

    final String profileOwnerName = widget.user?.userName ?? '';

    final String loginUserName = Get.find<AuthController>().user.value?.name ?? '';
    ...
    final reactedReels = controller.reelsList.where((reel) {
      ...
      if (isMyProfile) {
        final bool isLikedByMe = reel.isLiked == true;
    ...
    final staticReactions = widget.user?.userReactions ?? [];
                              ...
                              child: Stack(
                                ...
                                children: [
                                  Image.network(
                                    ...
                                    width: 64, height: 48,
                                    fit: BoxFit.cover,
                                    errorBuilder: (context, error, stackTrace) => Container(width: 64, height: 48, color: Colors.grey[800]),
                                  ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(reaction['time'] ?? "",
                          style: AppTextStyles.display.copyWith(color: AppColors.textOnDark, fontSize: 10)),
                      const SizedBox(height: 12),
                      GestureDetector(
                        onTap: () {
